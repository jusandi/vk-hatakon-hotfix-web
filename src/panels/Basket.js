...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  ...
  const basketFormState = JSON.parse(localStorage.getItem('basketFormState') || "{}");

  let [ faster, setFaster ] = useState('faster' in basketFormState ? basketFormState.faster : true);
  let [ time, setTime ] = useState('time' in basketFormState ? basketFormState.time : '');
  let [ selfService, setSelfService ] = useState('selfService' in basketFormState ? basketFormState.selfService : false);
  const area = foodAreas.filter(area => area.id === areaId)[0];
  const item = area.items.filter(item => item.id === itemId)[0];

  useMemo(() => {
    localStorage.setItem('basketFormState', JSON.stringify({ faster, time, selfService }));
